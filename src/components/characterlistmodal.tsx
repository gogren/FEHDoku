...
const CharacterListModal: React.FC<InfoModalProps> = ({ isVisible, onClose, children, generate_ten_chars, getTitleStrings, getWarnings, handleChosenCharacter, inputText, charList, usedChars }) => {
                ...
                <div className="bg-white p-2 rounded-md shadow-md flex flex-col">
                    ...
                    <div className="text-black place-self-center text-xl pb-4">
                        <form className="flex flex-col items-center">
                            <div className="text-center text-black pb-1">{getTitleStrings()}</div>
                            {getWarnings().map((warning) => {
                                console.log()
                                if (warning != "") {
                                return (
                                    <div className="bg-red-100 rounded-lg pr-2 mb-1 flex flex-row text-sm">
                                        <div className="bg-red-800 rounded-full flex items-center justify-center w-5 h-5">
                                            <span className="text-white text-sm font-bold">!</span>
                                        </div>
                                        <div className="text-red-800 ml-2">{warning}</div>
                                    </div>
                                )}
                            })}
                        ...
                        <div className="flex flex-col pt-3 max-h-[350px] overflow-y-auto border-b-4 rounded-b-lg mt-2 rounded-md">
